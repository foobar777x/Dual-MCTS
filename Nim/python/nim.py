...
class NimState(object):
    """A python-only version of the Nim state.
  ...
  """
    ...
    def apply_action(self, action):
        ...
        if self._cur_player == 0:
            self._board[self.coord(action)] = "x"
            for i in range(action):
                if self._board[(0, i)] == ".":
                    self._board[(0, i)] = "x"

        else:
            self._board[self.coord(action)] = "o"
            for i in range(action):
                if self._board[(0, i)] == ".":
                    self._board[(0, i)] = "o"
        ...
        # if self.line_exists():
        if self._board[(0, _NUM_COLS - 1)] == "x" or self._board[(0, _NUM_COLS - 1)] == "o":
    ...
    def clone(self):
        cloned_state = NimState(self._game)
  ...
  """
    ...
    def get_type(self):
        return pyspiel.GameType(
            short_name="python_nim",
            long_name="Python Nim",
